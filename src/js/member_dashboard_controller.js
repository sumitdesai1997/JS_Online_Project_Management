...
userRelatedProjectList.forEach(function callback(project, index) {
  ...
  let markup = "<tr><td>" + (index+1) + "</td><td><a>"+ project.projectName +"</a><br/><small>Created "+ project.startDate +"</small></td><td><ul class='list-inline'>"+ imgAvatar +"</ul></td><td >" + getNumberOfDays(new Date(), project.endDate) +" Days</td><td class='project-state'><span class='badge badge-success'>"+ project.projectStatus +"</span></td><td class='project-actions text-right'><button class='btn btn-primary btn-sm'onclick='goToProjectRelatedTask("+index+")'><i class='fas fa-folder'></i>  View Task</button></td></tr>";
